...
```
parse 函数的代码很长，贴一遍对理解没有好处，先把它拆成伪代码的形式，方便我们对整体流程先有一个大致的了解。接下来我们就来分解分析每段伪代码的作用
...
<img src='./images/advance1.png'>
调用 `advance` 函数：
```
advance(4)
```
<img src='./images/advance2.png'>
...
最后调用了`options.start`回调函数，并传入一些参数，这个回调函数的作用(创建AST树)稍后会详细介绍。
...
最后调用了 `options.end` 回调函数，并传入一些参数，这个回调函数的作用稍后详细介绍。
* 文本
```
let text, rest, next
if (textEnd >= 0) {
