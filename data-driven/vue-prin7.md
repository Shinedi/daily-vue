...
### update
---
在`mountcomponent`中通过渲染watcher调用`vm._update(vm._render(), hydrating)`
...
接着再调用`invokeCreateHooks`方法执行所有的create的钩子并把 `vnode`push 到 `insertedVnodeQueue `中。
```
if (isDef(data)) {
  invokeCreateHooks(vnode, insertedVnodeQueue)
}
